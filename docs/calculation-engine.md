...
| `diagramToJson` | `(nodes, edges) => string` | Returns `JSON.stringify({ nodes, edges, version: 2 }, null, 2)`. |
| `diagramToDrawio` | `(nodes, edges) => string` | Builds draw.io-compatible `<mxfile>` XML with rich HTML labels and embedded SVG icons. |

### draw.io Export Details

The draw.io export produces high-fidelity output that mirrors the designer:

- **VPC containers** — `html=1` rich labels with provider badge (e.g. "VNET", "VPC"), label, and CIDR in monospace. Provider-specific border colors and dash patterns. Azure uses solid borders, others dashed.
- **Subnet containers** — `html=1` labels with colored dot indicator, label, CIDR, and hosts count badge. Dotted borders (`dashPattern=2 2`).
- **Cloud resources** — Embedded SVG icons via `shape=image` with `data:image/svg+xml` data URIs from `drawio-icons.ts`. Falls back to styled rectangles when no icon available.
- **Edges** — `curved=1;orthogonalLoop=1;jettySize=auto` for cleaner routing with Solarized cyan stroke.
- **Background** — `background="#002b36"` dark theme on the mxGraphModel.

### draw.io Shape Mapping (Legacy Fallback)
...
## drawio-icons.ts — Draw.io SVG Icon Registry

Provides raw SVG strings for embedding cloud resource icons in draw.io exports.

| Function | Signature | Description |
|----------|-----------|-------------|
| `getDrawioSvg` | `(provider, resourceType) => string \| undefined` | Returns raw SVG XML string. Tries provider-specific → generic mapping → generic fallback. |
| `getDrawioSvgDataUri` | `(provider, resourceType) => string \| undefined` | Returns `data:image/svg+xml,...` URI for draw.io `shape=image`. |

Raw SVG strings live in `src/lib/drawio-svg-strings/`:
- `aws.ts`, `azure.ts` — Auto-generated by `pnpm generate-icons`
- `gcp.ts`, `generic.ts` — Hand-written
