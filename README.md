...
- **Network Designer** — Visual drag-and-drop network topology diagram at `/designer`, with auto-layout from splitter data, bidirectional state-preserving navigation between calculator and designer, custom subnet and resource nodes (routers, switches, firewalls, servers, databases, load balancers, cloud resources), properties panel for editing nodes, arrange tools (auto-layout, align, distribute), export to PNG/SVG/JSON/draw.io, localStorage persistence, keyboard shortcuts (Escape, Cmd+E, Cmd+S), and floating toolbar — powered by React Flow
