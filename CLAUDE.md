...
`src/lib/export.ts` generates JSON, CSV, and Terraform HCL (AWS, Azure, GCP). `src/lib/export-cli.ts` generates CLI commands (AWS CLI, Azure CLI, gcloud). `src/lib/export-diagram.ts` generates PNG, SVG, JSON, and draw.io XML from designer diagrams; `getDiagramsNetUrl()` encodes draw.io XML via `pako.deflateRaw` + base64 into a `https://app.diagrams.net/#R{data}` URL for direct editing. `src/lib/syntax-highlight.ts` provides regex-based tokenization for `hcl`, `json`, `shell`, `csv`, and `xml` with Solarized colors. The export UI uses two-tier tabs (Data, CLI, Terraform, Share) with provider selector for CLI/Terraform categories. The draw.io export uses `html=1` rich labels for VPC containers (provider badge + label + CIDR) and subnet containers (color dot + label + CIDR + hosts badge), embedded SVG data URIs (`shape=image`) for cloud resource icons via `src/lib/drawio-icons.ts`, and `background="#002b36"` dark theme. The designer export modal offers "Download .drawio" and "Open in diagrams.net" buttons side by side. Raw SVG strings live in `src/lib/drawio-svg-strings/` — AWS and Azure are auto-generated by `pnpm generate-icons`, GCP and generic are hand-written.
