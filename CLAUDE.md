...
- **`src/hooks/`** — Side-effect hooks. `use-url-sync.ts` syncs store ↔ URL path bidirectionally (with legacy hash migration). `use-designer-url-sync.ts` reads `?from=`, `&split=`, and `&provider=` params on `/designer` route; merges new subnets into existing saved diagrams (same VPC CIDR) or generates fresh layouts, then writes the canonical `?from&split&provider` URL via `buildDesignerUrl()` so refresh/bookmark keeps context (`?d=` share payloads are also replaced with canonical params). `use-calculator-href.ts` returns a calculator URL from designer state via `extractDesignerState()` and `encodeState()`, enabling state-preserving back-navigation from designer to calculator. `use-keyboard-shortcuts.ts` handles `/` to open command palette, Cmd/Ctrl+E to toggle the calculator export modal, arrows to adjust prefix when input focused. `use-clipboard.ts` wraps clipboard API with feedback state. `use-rdap-lookup.ts` fetches RDAP data for public IPs with debounce, abort, and caching. `use-diagram-persistence.ts` auto-saves/loads designer state to/from localStorage with 1s debounce, runs migration from v1→v2 on load, and keeps the `/designer` URL canonical with the diagram on every save. `use-designer-shortcuts.ts` handles Escape (deselect/close), Cmd+E (export modal), Cmd+S (manual save), 1/2/3 (layer switching) in designer. `use-touch-detect.ts` detects touch devices for tap-to-place mode. `use-document-title.ts` updates `document.title` dynamically based on calculator state (CIDR loaded, splitter mode, supernet).
...
`App.tsx` checks pathname — if `/designer`, renders `<DesignerPage>` (separate React Flow canvas); otherwise renders the calculator `<Layout>`. Calculator: `useUrlSync()` migrates legacy hash URLs then initializes store from path → Zustand store drives all components → store changes trigger URL path updates. Designer: `useDesignerUrlSync()` reads `?from=` and `&split=` params → if a saved diagram exists in localStorage with the same VPC CIDR, **merges** new subnets into the existing diagram (preserving user-added resources and positions); otherwise `generateInitialLayout()` creates nodes/edges from scratch → `designer-store` drives React Flow canvas. Navigation between views is bidirectional and state-preserving: the Header, command palette, and SplitterToolbar always build designer URLs from calculator store state (`?from=&split=`). `DesignerHeader` uses `useCalculatorHref()` to build the calculator URL from diagram nodes (the logo is the single state-preserving back link). The Supernet tool's "View details" hands off via `loadSupernetResult()`, which records a `handoff` in the calculator store; `CidrInput` shows a dismissible "Loaded supernet result · Undo" notice.
...
The `dark` class is toggled on `<html>` by the theme store. The visual design uses the **Solarized** color system exposed as **semantic tokens** in `index.css`: CSS custom properties on `:root`/`.dark` are surfaced via `@theme inline`, so utilities resolve at the use-site and flip with dark mode automatically — components need **no `dark:` prefixes** for themed colors.
...
|---------|------|-------|------|
| `canvas` | Page background | `#fdf6e3` (base3) | `#002b36` (base03) |
| `surface` | Cards, chips | `#eee8d5` (base2) | `#073642` (base02) |
| `well` | Inputs/inset areas on a surface | `#fdf6e3` (base3) | `#002b36` (base03) |
| `ink` | Emphasized text | `#586e75` (base01) | `#93a1a1` (base1) |
| `ink-body` | Body text | `#657b83` (base00) | `#839496` (base0) |
| `ink-muted` | Secondary text | `#93a1a1` (base1) | `#586e75` (base01) |
| `line` | Borders/dividers | `#93a1a1` (base1) | `#586e75` (base01) |

Usage: `bg-surface text-ink border-line/20`. Micro-labels use `text-sol-base01` (mode-invariant by design) via the `SectionLabel` primitive.

**Accent tokens (same in both modes):** `sol-cyan` (primary/links), `sol-blue`/`network-bit`, `sol-magenta`/`host-bit`, `sol-green` (success), `sol-red` (error), `sol-yellow` (warning), `sol-violet`, `sol-orange`. Mode-invariant base scale available as `sol-base03…sol-base3`.

**Shared primitives (`src/components/shared/`):** `Button` (variants soft/primary/ghost/danger/outline, sizes xs/sm/md, `active` state, renders `<a>` with `href`), `IconButton`, `Input`/`Select`/`Textarea` (one focus treatment), `SectionLabel`/`LabelValue` (label-over-value pattern), `SegmentedControl` (mode toggles, provider pickers, tab bars), `Modal` (chrome default/none, used by both export dialogs and the command palette), `ThemeToggle`, plus the older `Badge` (colors green/violet/yellow/cyan/red/neutral), `CopyButton`, `AnimatedCard`, `CollapsibleSection`, `Drawer`. Motion presets (`EASE`, `DUR`, `fadeIn`, `cardEntrance`, `modalPop`, `collapse`, `drawerSpring`) live in `src/components/shared/motion.ts`. Both app shells compose `src/components/layout/HeaderBar.tsx`.
...
`src/lib/commands.ts` defines the `Command` interface and a static list of ~23 commands across 4 categories (Navigate, Tools, Export, Actions — including `open-export`, which opens the calculator export modal). `filterCommands()` filters by label/description/keywords. `detectCidrInput()` detects CIDR input in the search query and returns a normalised CIDR string. The `CommandPalette` component (`src/components/command-palette/`) renders inside the shared `Modal` (`chrome="none"`, top-aligned) triggered by pressing `/` (outside inputs) or clicking the header button. It supports keyboard navigation (arrows, Enter, Escape), CIDR auto-detection, and grouped results. Export commands with `requiresResult: true` are hidden when no CIDR is loaded. The `inputMode` state (`'guided' | 'cidr'`) lives in the calculator store and is shared between `CidrInput` and the command palette's mode-switching commands.

### Export Formats

`src/lib/export.ts` generates JSON, CSV, and Terraform HCL (AWS, Azure, GCP). `src/lib/export-cli.ts` generates CLI commands (AWS CLI, Azure CLI, gcloud). `src/lib/export-diagram.ts` generates PNG (transparent, 2x), SVG (transparent), JSON (with `cloudProvider`), and draw.io XML from designer diagrams; `parseDiagramJson()` validates imported JSON structure; `compressDiagramState()`/`decompressDiagramState()` encode diagram state via `pako.deflateRaw` + base64url for share URLs (`?d=` param); `getShareUrl()` produces `/designer?d={compressed}` URLs; `getDiagramsNetUrl()` encodes draw.io XML via `pako.deflateRaw` + base64 into a `https://app.diagrams.net/#R{data}` URL for direct editing. `src/lib/diagram-saves.ts` provides named localStorage saves CRUD (`getSavedDiagrams`, `saveDiagram`, `loadSavedDiagram`, `deleteSavedDiagram`) with manifest at `subnet-designer-saves` and per-save at `subnet-designer-save-{id}`. `src/lib/syntax-highlight.ts` provides regex-based tokenization for `hcl`, `json`, `shell`, `csv`, and `xml` with Solarized colors. The calculator export UI is a shared-`Modal` dialog (`src/components/export/ExportModal.tsx` wrapping `ExportMenuContent`) with tabs (Data, CLI, Terraform, Share) and a provider selector for CLI/Terraform; it opens from the "Export & Share" trigger card, the command palette, or Cmd/Ctrl+E. The draw.io export uses `html=1` rich labels for VPC containers (provider badge + label + CIDR) and subnet containers (color dot + label + CIDR + hosts badge), embedded SVG data URIs (`shape=image`) for cloud resource icons via `src/lib/drawio-icons.ts`, and `background="#002b36"` dark theme. The designer export/import modal (Image | Data | draw.io | Share tabs) offers transparent PNG/SVG export, JSON download/import with validation and migration, named localStorage saves with load/delete, draw.io download + open in diagrams.net, and compressed share URL generation. Raw SVG strings live in `src/lib/drawio-svg-strings/` — AWS and Azure are auto-generated by `pnpm generate-icons`, GCP and generic are hand-written.
...
- **Properties Panel** — `PropertiesPanel` is an animated inline sidebar (320px, `motion/react` AnimatePresence) inside the flex row on desktop; below `md` (768px) it renders as an overlay `Drawer` (`variant="overlay"`). Includes a Delete Node button in the footer.
...
- **Tablet / mobile** — No hard block. Below `md` the palette opens as an overlay `Drawer` from a floating "Add" button (items use tap-to-place via `forceTap`), and the properties panel becomes an overlay Drawer. Below 480px a dismissible soft banner (sessionStorage) notes the desktop is recommended.
- **Export / Import** — Shared-`Modal` dialog with 4 tabs (Data, Image, Diagrams.net, Share). Image exports transparent PNG/SVG. Data tab has JSON download/import (with validation via `parseDiagramJson`) and named localStorage saves (via `src/lib/diagram-saves.ts`). draw.io tab exports XML with `html=1` rich labels, embedded SVG data URIs, `curved=1` edge routing, and `background="#002b36"`. Share tab generates compressed `?d=` URLs via `compressDiagramState()`. Store action `importDiagram` loads external data with `isDirty: true`.
...
- Use semantic tokens (`bg-surface text-ink border-line/20`, accents `text-sol-cyan` etc.), never Tailwind color names (`text-cyan-500`) and never raw hex in Tailwind classes. Raw hex is allowed only in `src/lib/` data that feeds exported artefacts (cloud-theme, drawio, syntax-highlight) and inline SVG/style attributes driven by data
- The semantic tokens flip with dark mode via `@theme inline` — do NOT add `dark:` twins for themed colors; `dark:` is only for genuinely asymmetric cases (e.g. `shadow-sm dark:shadow-none`)
- Element-level rules in `index.css` must live in `@layer base`, or they silently beat Tailwind utilities (unlayered CSS wins over `@layer`)
- Don't pass `hidden`/display utilities via Button's `className` (its base `inline-flex` wins) — wrap the Button in a `<span className="hidden sm:inline-flex">` instead
...
- Worker (`worker/`) has its own `tsconfig.worker.json` NOT referenced in `tsc -b` — wrangler bundles it separately
- OG image generation (`/og/*`) uses resvg WASM which may fail in local `wrangler dev` — this is expected; works in production
- Worker imports from `src/lib/` must be pure functions (no `window`, no DOM, no React)
